...
class OKXExchange(BaseExchange):
    ...
    async def GetDepth(self, symbol: str) -> OrderBook:
        """获取市场深度"""
        try:
            orderbook = await self.exchange.fetch_order_book(f"{symbol.replace('_', '/')}")
            return OrderBook(
    ...
    async def CancelOrder(self, symbol: str, order_id: str) -> bool:
        """取消订单"""
        try:
            await self.exchange.cancel_order(order_id, symbol.replace('_', '/'))
            return True
        except Exception as e:
    ...
    async def GetOrder(self, symbol: str, order_id: str) -> Dict[str, Any]:
        """获取订单信息"""
        try:
            order = await self.exchange.fetch_order(order_id, symbol.replace('_', '/'))
            return {
                'id': order['id'],
                'price': float(order['price']),
                'amount': float(order['amount']),
    ...
    async def GetOrders(self, symbol: str) -> List[Dict[str, Any]]:
        """获取所有未完成订单"""
        try:
            orders = await self.exchange.fetch_open_orders(symbol.replace('_', '/'))
            return [{
                'id': order['id'],
                'price': float(order['price']),
                'amount': float(order['amount']),
