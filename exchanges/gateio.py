    ...
    async def GetDepth(self, symbol: str) -> OrderBook:
        ...
        try:
            ...
            if market_symbol not in self.exchange.markets:
                Log(f"交易对不存在: {market_symbol}")
                return OrderBook(Asks=[], Bids=[])
            
            # 获取深度数据
            orderbook = await self.exchange.fetch_order_book(market_symbol)
